...
class LGCN_layer(MessagePassing):
    def __init__(self):
...
class LGCN_Network(torch.nn.Module):
    def __init__(self, num_layers):
        super(LGCN_Network, self).__init__()
        self.layers = torch.nn.ModuleList([LGCN_layer() for _ in range(num_layers)])
    
    def forward(self, x, edge_index):
        emb = [x]
        for layer in self.layers:
            emb.append(layer(emb[-1], edge_index))
        emb = torch.stack(emb, dim=-1)
        emb = emb.mean(dim=-1)
        return emb
    
class LightGCN_PyG(BasicModel):
    def __init__(self, 
                 config:dict, 
                 dataset:BasicDataset):
